...
# Separate data for Italy and Spain
italy_data <- subset(covid, country == "Italy")
spain_data <- subset(covid, country == "Spain")

# Define the exponential growth function
exponential_growth <- function(t, A, r) {
  A * exp(r * t)
}

# Fit the exponential growth model for Italy
italy_model <- nlsLM(deaths ~ exponential_growth(days_since_first_death, A, r), 
                     data = italy_data, 
                     start = c(A = max(italy_data$deaths), r = 0.01))
...
spain_model <- nlsLM(deaths ~ exponential_growth(days_since_first_death, A, r), 
                     data = spain_data, 
                     start = c(A = max(spain_data$deaths), r = 0.01))
...
# Combine Italy and Spain data into one dataframe
combined_covid_data <- rbind(italy_data, spain_data)

# Create a line graph for both countries on the same plot
ggplot(combined_covid_data, aes(x = days_since_first_death, y = deaths, col = country)) +
  geom_line() +
  labs(x = "Days Since First Death", y = "Reported Daily Deaths", title = "Reported Daily Deaths Over Time") +
  theme_minimal() +
  scale_color_manual(values = c("lightblue4", "salmon"))
